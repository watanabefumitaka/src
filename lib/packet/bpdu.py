...
class bpdu(packet_base.PacketBase):
    ...
    def parser(cls, buf):
        ...
        assert bpdu_type in cls._BPDU_TYPES
        bpdu_cls = cls._BPDU_TYPES[bpdu_type]
...
class ConfigurationBPDUs(bpdu):
    ...
    def encode_bridge_id(priority, system_id_extension, mac_address):
        ...
        return ((priority >> 12) + (system_id_extension << 4)
                + (mac_addr << 16))

    @staticmethod
    def _decode_port_id(port_id):
        priority = (port_id << 4) & 0xf0
        port_number = (port_id >> 4) & 0xfff
...
class RstBPDUs(ConfigurationBPDUs):
    ...
    def parser(cls, buf):
        ...
        return get_cls, next_type, buf[RstBPDUs._PACK_LEN:]
