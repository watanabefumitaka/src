...
""" How to decide the port roles.
     ...
     Root bridge: a bridge has smallest bridge ID is chosen as a root.
                  it sends original config BPDU.
     Non Root bridge: forwards config BPDU received from the root bridge.
     ...
     ROOT_PORT: the nearest port to a root bridge of the bridge.
                it is determined by the cost of the path, etc.
     DESIGNATED_PORT: the port of the side near the root bridge of each link.
                      it is determined by the cost of the path, etc.
     NON_DESIGNATED_PORT: the port other than a ROOT_PORT and DESIGNATED_PORT.
"""
...
class EventPortStateChange(event.EventBase):
    def __init__(self, dp, port):
        ...
        self.dp = dp
        self.port_no = port.data.port_no
...
class Port(object):
    ...
    def _get_next_state(self):
        next_state = {PORT_STATE_DISABLE: None,
                      PORT_STATE_BLOCK: PORT_STATE_LISTEN,
